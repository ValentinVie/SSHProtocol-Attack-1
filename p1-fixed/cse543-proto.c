...
int client_authenticate( int sock, unsigned char **session_key )
{
    ...
    *session_key = (unsigned char *) malloc(session_key_length);
    ...
    //-- wait for the message containing the nonce
    ProtoMessageHdr hdr32;
    char block32[MAX_BLOCK_SIZE];
    bzero(block32, MAX_BLOCK_SIZE); //clear 
    if (wait_message( sock, &hdr32, block32, SERVER_CONFIRM ) == -1){
        printf("client_authenticate wait_message SERVER_CONFIRM error.\n");
        return -1;
    }
    byte_read += hdr32.length;
    
    //decrypt the nonce
    int len_nonce_buffer = 0;
    unsigned char *nonce_buffer = (unsigned char *)malloc(hdr32.length - 16 - TAGSIZE);
    bzero(nonce_buffer, hdr32.length - 16 - TAGSIZE);

    if (nonce_buffer == NULL){
        printf("Couldn't allocate nonce, Message 5, client_authenticate,\n");
        return -1;
    }

    if(decrypt_message(block32, hdr32.length, (const char *) *session_key, nonce_buffer, &len_nonce_buffer) ==-1){
        printf('decrypt_message failed in client_authenticate.\n');
        return -1;
    }

    //create the int object containing the nonce value
    unsigned long nonce = 0;
    memcpy((char*)&nonce, nonce_buffer, sizeof(unsigned long));
    //--------------- END FIX MESSAGE 5

    //--------------- FIX MESSAGE 6
    //add 1 to the nonce value
    nonce += 1;
    memcpy(nonce_buffer, (char*)&nonce, sizeof(unsigned long)); //copy back to the buffer

    //encrypt nonce + 1 buffer
    int len_E_nonce_added1 = 0;
    unsigned char *E_nonce_added1 = (unsigned char *) malloc(sizeof(nonce) + TAGSIZE + 16); // the encrypted buffer containing epoch

    rc = encrypt_message( (unsigned char *) nonce_buffer, sizeof(nonce), *session_key, E_nonce_added1, &len_E_nonce_added1); //encrypt epoch and place it in E_epoch
    if(rc == -1){
        printf("encrypt_message Message 6 failed in client_authenticate.\n");
        return -1;
    }

    //Send the nonce + 1 encrypted buffer
    ProtoMessageHdr hdr42;
    hdr42.msgtype = CLIENT_CONFIRM;
    hdr42.length = len_E_nonce_added1;
    if(send_message(sock, &hdr42, E_nonce_added1) == -1){
        printf("client_authenticate send_message CLIENT_CONFIRM error.\n");
        return -1;
    }
...
int client_secure_transfer( struct rm_cmd *r, char *fname, char *address ) 
{
	...
	if (client_authenticate( sock, &key ) == -1){
        return -1;
    }
...
int server_protocol( int sock, char *pubfile, EVP_PKEY *privkey, unsigned char **enckey )
{
    ...
    */
    ...
    if(send_message(sock, &hdr4, E_epoch) == -1){
        printf("server_protocol send_message SERVER_INIT_ACK error.\n");
        return -1;
    }
...
//--------------- FIX MESSAGE 5
    unsigned long real_nonce = 0; //the int value of the nonce
    unsigned char *nonce_buffer; // the nonce buffer
    unsigned char *E_nonce; // the encrypted Nonce (5 bytes is enough)
    int len_E_nonce;
    int len_nonce = (int) sizeof(unsigned long);

    nonce_buffer = (unsigned char *) malloc(sizeof(unsigned long));
    bzero(nonce_buffer, sizeof(unsigned long));
    E_nonce = (unsigned char *) malloc(sizeof(unsigned long) + TAGSIZE + 16);\
    bzero(E_nonce, sizeof(unsigned long) + TAGSIZE + 16);

    if (nonce_buffer == NULL || E_nonce == NULL){
        printf("Couldn't allocate nonce or E_nonce, Message 5, server_protocol.\n");
        return -1;
    }

    //Generate the nonce
    if (generate_pseudorandom_bytes( nonce_buffer, (unsigned int) len_nonce) == -1){
        printf("Couldn't generate nonce, Message 5, server_protocol.\n");
        return -1;
    }
    memcpy((char*)&real_nonce, nonce_buffer, sizeof(unsigned long));

    //Encrypt the nonce
    if(encrypt_message( nonce_buffer, len_nonce, *enckey, E_nonce, &len_E_nonce) == -1){
        printf("Couldn't encrypt nonce, Message 5, server_protocol.\n");
        return -1;
    }
    ...
    //Send the nonce
    ProtoMessageHdr hdr32;
    hdr32.msgtype = SERVER_CONFIRM;
    hdr32.length = len_E_nonce;
    if(send_message(sock, &hdr32, E_nonce) == -1){
        printf("server_protocol send_message Message 5 error.\n");
        return -1;
    }
    ...
    //Receive nonce + 1
    ProtoMessageHdr hdr42;
    unsigned char E_nonce_added1[MAX_BLOCK_SIZE]; // will contain the confirmation (encrypted epoch) if the protocol went well
    bzero(E_nonce_added1, MAX_BLOCK_SIZE); //clear 
    if (wait_message( sock, &hdr42, E_nonce_added1, CLIENT_CONFIRM ) == -1){
        printf("server_protocol  wait_message CLIENT_CONFIRM error.\n");
        return -1;
    }
    byte_read += hdr42.length;

    //decrypt the nonce
    unsigned long nonce_added1 = 0;
    int len_nonce_added1 = 0;

    unsigned char *nonce_added1_buffer = (unsigned char *)malloc(hdr42.length - 16 - TAGSIZE);
    bzero(nonce_added1_buffer, hdr42.length - 16 - TAGSIZE);
    if (nonce_added1_buffer == NULL){
        printf("Couldn't allocate nonce_added1, Message 6, server_protocol,\n");
        return -1;
    }

    if(decrypt_message(E_nonce_added1, hdr42.length, *enckey, nonce_added1_buffer, &len_nonce_added1) ==-1){
        printf('decrypt_message failed, Message 6, server_protocol.\n');
        return -1;
    }

    //copy the decrypted in an unsigned long variable
    memcpy((char*)&nonce_added1, nonce_added1_buffer, sizeof(unsigned long));

    //check if the nonces are equals
    if (!(nonce_added1 == real_nonce + 1)){
        printf("Nonce invalid, replay attack detected: ABORT !\n");
        return -1;
    }
    else{
        printf("Nonce valid, all good !\n");
    }

    free(nonce_added1_buffer);
    //--------------- END FIX MESSAGE 6
    ...
    while ( !errored )
	{
		...
		else
		{
			...
			if ( (newsock = server_accept(server)) != -1 )
			{
				...
				if (server_protocol( newsock, pubfile, privkey, &key ) == -1){
                    printf("Protocol failed server_protocol\n.");
                    return -1;
                }
